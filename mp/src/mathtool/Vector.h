...
namespace mathtool{
    ...
    public:
        ...
        Vector<T,3> toVector3d() const;					//to 3d
        Vector<T,2> toVector2d() const;					//to 2d
        double angleTo(const Vector& v2);				// angleTo
    ...
    template<class T, int D >
	Vector<T,D>& operator*=(T& s, Vector<T,D>& v)
	{
		return v*=s;
	}
    ...
    template<class T, int D >
    Vector<T,3> Vector<T,D>::toVector3d() const
	{
    	Vector<T,3> vector(v[0], v[1], v[2]);

    	return vector;
	}

    template<class T, int D >
    Vector<T,2> Vector<T,D>::toVector2d() const
	{
    	Vector<T,2> vector(v[0], v[1]);

    	return vector;
	}

    // get CCW rotation angle from this to v2
	template<class T, int D >
	double Vector<T,D>::angleTo(const Vector<T,D>& v2)
	{
		double dot = (*this)*v2;
		Vector<T,D> cross = (*this)%v2;
